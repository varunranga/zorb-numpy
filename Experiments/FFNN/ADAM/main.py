import argparse
parser = argparse.ArgumentParser(description = 'Run the ADAM algorithm to train a neural network on a particular dataset.')
...
parser.add_argument('-lr', '--learning-rate', type = float, default = 1e-3, help = 'Learning rate for the Adam update rule')
...
if args.load:
	...
	network = ADAM.Network.load_network(args.load)

else:

	network = ADAM.Network(input_dimension = dataset.input_dimension)
...
if not args.no_train:
	...
	start_time = time()
	network.fit(X = dataset.train_x, Y = dataset.train_y, learning_rate = args.learning_rate)
	end_time = time()
